...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const aidRecords = await ds.query('SELECT * FROM aid_records WHERE headOfHouseholdId = ? ORDER BY aidDate DESC', [id]);

    return successResponse({ ...mapRow(rows[0]), familyMembers: members, aidRecords });
  } catch (error) {
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await ds.query('SELECT * FROM residents WHERE id = ?', [id]);
    if (!existing.length) return errorResponse('المقيم غير موجود', 404);

    if (body.nationalId && body.nationalId !== existing[0].nationalId) {
      const dup = await ds.query('SELECT id FROM residents WHERE nationalId = ? AND id != ?', [body.nationalId, id]);
      if (dup.length) return errorResponse('رقم الهوية مسجل مسبقاً', 409);
    }

    if (body.headOfHouseholdId) {
      const head = await ds.query("SELECT relationToHead FROM residents WHERE id = ?", [body.headOfHouseholdId]);
      if (!head.length || head[0].relationToHead !== 'head') {
        return errorResponse('المحدد كرب أسرة ليس رب أسرة فعلياً', 400);
      }
    }
    ...
    await ds.query(
      `UPDATE residents SET nationalId=?, firstName=?, fatherName=?, grandfatherName=?, familyName=?,
       ...
       isPregnant=?, isBreastfeeding=?, isActive=? WHERE id=?`,
      [
        ...
        body.hasChronicDisease !== undefined ? (body.hasChronicDisease ? 1 : 0) : current.hasChronicDisease,
        body.chronicDiseaseDescription ?? current.chronicDiseaseDescription,
        body.hasDisability !== undefined ? (body.hasDisability ? 1 : 0) : current.hasDisability,
    ...
    const updated = await ds.query('SELECT * FROM residents WHERE id = ?', [id]);
    return successResponse(mapRow(updated[0]));
  } catch (error) {
    return errorResponse('فشل في تحديث البيانات', 500, String(error));
  }
}
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    const ds = await getDataSource();
    const existing = await ds.query('SELECT id FROM residents WHERE id = ?', [id]);
    if (!existing.length) return errorResponse('المقيم غير موجود', 404);
    await ds.query('UPDATE residents SET isActive = 0 WHERE id = ?', [id]);
